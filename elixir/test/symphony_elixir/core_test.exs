defmodule SymphonyElixir.CoreTest do
  ...
  test "planning issues do not chain in-process continuation turns" do
    issue = %Issue{
      id: "planning-continue-1",
      identifier: "MT-701",
      title: "Stay in planning",
      state: "Planning"
    }

    fetcher = fn ["planning-continue-1"] ->
      {:ok, [%Issue{issue | updated_at: ~U[2026-03-06 12:15:00Z]}]}
    end

    assert {:done, %Issue{state: "Planning"} = refreshed_issue} =
             AgentRunner.continue_with_issue_for_test(issue, fetcher)

    assert refreshed_issue.identifier == "MT-701"
  end
